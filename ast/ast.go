...
type PrefixExpression struct {
	Token    token.Token // the prefix token, ex. ! or -
	Operator string
	Right    Expression
}
...
type InfixExpression struct {
	Token    token.Token // the operator token, ex. + or -
	Left     Expression
	Operator string
	Right    Expression
}
...
type Boolean struct {
	Token token.Token // the boolean token, ex. TRUE or FALSE
	Value bool
}
...
// The consequence and alternative are block statements that are executed
// based on the value of the condition.
// The alternative BlockStatement is optional.
...
type FunctionLiteral struct {
	Token      token.Token // the function token, token.FUNCTION
